...
public void drawTree()
{
  ...
  treeR = treeColors.get(colorChoice)[0];
  treeG = treeColors.get(colorChoice)[1];
  treeB = treeColors.get(colorChoice)[2];
  ...
  for (int i = 0; i < random(50, 70); i++)
  {
    ...
    drawBranch(leftOrRight);
  }
}

public void drawBranch(int direction)
{
  if (direction == 0)
    {
...
public void drawLeaves(int rowAmount, int direction)
{
  if(direction == 0)
  {
    for (int leafRow = 0; leafRow < rowAmount; leafRow++)
    {
